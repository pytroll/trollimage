"""Module for testing the image module."""
...
class TestEmptyImage(unittest.TestCase):
    ...
    def test_invert(self):
        ...
        for mode in self.modes:
            ...
            self.assertRaises(ValueError, self.img.invert, [True, False, True, False, True, False, True, False])
        self.img.convert(oldmode)
    ...
    def test_putalpha(self):
        ...
        for mode in self.modes:
            ...
            self.img.convert(mode)
            self.assertRaises(ValueError, self.img.putalpha, np.random.rand(3, 2))
...
class TestImageCreation(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        self.assertRaises(TypeError, image.Image, channels=random.randint(1, 1000))
        self.assertRaises(TypeError, image.Image, channels=random.random())
        self.assertRaises(TypeError, image.Image, channels=random_string(random.randint(1, 10)))
        ...
        one_channel = np.random.rand(random.randint(1, 10), random.randint(1, 10))
        ...
        for mode in self.modes:
            ...
            self.assertRaises(ValueError, image.Image, mode=mode, fill_value=0, color_range=((0, (1, 2))))
            ...
            self.assertRaises(
                ...
                mode=mode,
                fill_value=0,
                color_range=((0, 0), (1, 2), (0, 0), (1, 2), (0, 0), (1, 2)),
            )

            # Regular image, too many channels

            self.assertRaises(ValueError, image.Image, channels=([one_channel] * (self.modes_len[i] + 1)), mode=mode)
            ...
            self.img[mode] = image.Image(channels=([one_channel] * (self.modes_len[i])), mode=mode)

            for nb_chan in range(self.modes_len[i]):
                self.assertTrue(np.all(self.img[mode].channels[nb_chan] == one_channel))
                self.assertTrue(isinstance(self.img[mode].channels[nb_chan], np.ma.core.MaskedArray))
...
class TestRegularImage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rand_img2 = image.Image(channels=[one_channel] * 3, mode="RGB", fill_value=(0, 0, 0))
        ...
        self.flat_img = image.Image(channels=[self.flat_channel], mode="L", fill_value=0)
    ...
    def test_convert(self):
        ...
        for mode1 in self.modes:
            ...
            for mode2 in self.modes:
                self.img.convert(mode1)
                self.assertEqual(self.img.mode, mode1)
                self.assertEqual(len(self.img.channels), self.modes_len[i])
                self.img.convert(mode2)
                self.assertEqual(self.img.mode, mode2)
                self.assertEqual(len(self.img.channels), self.modes_len[j])

                self.rand_img2.convert(mode1)
                self.assertEqual(self.rand_img2.mode, mode1)
                self.assertEqual(len(self.rand_img2.channels), self.modes_len[i])
                if mode1 not in ["P", "PA"]:
                    self.assertEqual(len(self.rand_img2.fill_value), self.modes_len[i])
                self.rand_img2.convert(mode2)
                self.assertEqual(self.rand_img2.mode, mode2)
                self.assertEqual(len(self.rand_img2.channels), self.modes_len[j])
                if mode2 not in ["P", "PA"]:
                    self.assertEqual(len(self.rand_img2.fill_value), self.modes_len[j])
                j = j + 1
            i = i + 1
        while True:
            randstr = random_string(random.choice(range(1, 7)))
    ...
    def test_stretch(self):
        ...
        for mode in "L":
            ...
            linear = np.array([[0.0, 1.00048852, 1.00048852], [1.00048852, 0.50024426, 0.50024426]])
            crude = np.array([[0, 1, 1], [1, 0.5, 0.5]])
            histo = np.array([[0.0, 0.99951171875, 0.99951171875], [0.99951171875, 0.39990234375, 0.39990234375]])
            self.img.stretch()
            ...
            self.assertTrue(np.all((self.img.channels[0] - crude) < EPSILON))
            self.img.stretch("histogram")
            self.assertTrue(np.all(np.abs(self.img.channels[0] - histo) < EPSILON))
            self.img.stretch((0.05, 0.05))
            self.assertTrue(np.all((self.img.channels[0] - linear) < EPSILON))
            self.assertRaises(ValueError, self.img.stretch, (0.05, 0.05, 0.05))
    ...
    def test_gamma(self):
        ...
        for mode in self.modes:
            ...
            self.img.gamma(0.5)
            for i in range(len(self.img.channels)):
                self.assertTrue(np.all(self.img.channels[i] - old_channels[i] ** 2 < EPSILON))
            self.img.gamma(1)
            for i in range(len(self.img.channels)):
                self.assertTrue(np.all(self.img.channels[i] - old_channels[i] ** 2 < EPSILON))
            ...
            # input a tuple
            self.assertRaises(ValueError, self.img.gamma, list(range(10)))
            self.assertRaises(ValueError, self.img.gamma, (0.2, 3.0, 8.0, 1.0, 9.0))
    ...
    def test_invert(self):
        ...
        for mode in self.modes:
            ...
            self.img.invert()
            for i in range(len(self.img.channels)):
                self.assertTrue(np.all(self.img.channels[i] == 1 - old_channels[i]))
            self.img.invert(True)
            for i in range(len(self.img.channels)):
                self.assertTrue(np.all(self.img.channels[i] - old_channels[i] < EPSILON))
            self.assertRaises(ValueError, self.img.invert, [True, False, True, False, True, False, True, False])
        self.img.convert(oldmode)
    ...
    def test_pil_image(self):
        ...
        oldmode = self.img.mode
        for mode in self.modes:
            if mode == "YCbCr" or mode == "YCbCrA" or mode == "P" or mode == "PA":
                continue
            self.img.convert(mode)
            pilimg = self.img.pil_image()
    ...
    def test_putalpha(self):
        ...
        for mode in self.modes:
            ...
            self.img.convert(mode)
            self.assertRaises(ValueError, self.img.putalpha, np.random.rand(4, 5))

        self.img.convert(oldmode)

    @unittest.skipIf(sys.platform.startswith("win"), "Read-only tmp dir not working under Windows")
    def test_save(self):
        """Save an image."""
        oldmode = self.img.mode
        for mode in self.modes:
            ...
            # permissions
            self.assertRaises(IOError, self.img.save, os.path.join(self.tempdir, "test.png"))
    ...
    def test_save_jpeg(self):
        ...
        # permissions
        self.assertRaises(IOError, self.img.save, os.path.join(self.tempdir, "test.jpg"))
    ...
    def test_replace_luminance(self):
        ...
        for mode in self.modes:
            if mode == "P" or mode == "PA":
                continue
            self.img.convert(mode)
            luma = np.ma.array([[0, 0.5, 0.5], [0.5, 0.25, 0.25]])
    ...
    def test_resize(self):
        ...
        self.img.resize((2, 3))
        res = np.array([[0, 0.5, 0.5], [0.5, 0.25, 0.25]])
        self.assertTrue(np.all(res == self.img.channels[0]))
    ...
    def test_merge(self):
        ...
        newimg = image.Image(np.array([[1, 2, 3, 4]]))
        self.assertRaises(ValueError, self.img.merge, newimg)
        newimg = image.Image(np.ma.array([[1, 2, 3], [4, 5, 6]], mask=[[1, 0, 0], [1, 1, 0]]), mode="L")

        self.img.convert("L")
        newimg.merge(self.img)
        self.assertTrue(np.all(np.abs(newimg.channels[0] - np.array([[0, 2, 3], [0.5, 0.25, 6]])) < EPSILON))
...
class TestFlatImage(unittest.TestCase):
    ...
    def setUp(self):
        """Set up the test case."""
        channel = np.ma.array([[0, 0.5, 0.5], [0.5, 0.25, 0.25]], mask=[[1, 1, 1], [1, 1, 0]])
    ...
    def test_stretch(self):
        ...
        self.img.stretch()
        self.assertTrue(self.img.channels[0].shape == (2, 3) and np.ma.count_masked(self.img.channels[0]) == 5)
        self.img.stretch("crude")
        self.assertTrue(self.img.channels[0].shape == (2, 3) and np.ma.count_masked(self.img.channels[0]) == 5)
...
class TestNoDataImage(unittest.TestCase):
    ...
    def setUp(self):
        """Set up the test case."""
        channel = np.ma.array([[0, 0.5, 0.5], [0.5, 0.25, 0.25]], mask=[[1, 1, 1], [1, 1, 1]])
        self.img = image.Image(channels=[channel] * 3, mode="RGB")
        self.modes = ["L", "LA", "RGB", "RGBA", "YCbCr", "YCbCrA", "P", "PA"]
...
def random_string(length, choices="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"):
    """Generate a random string with elements from *set* of the specified *length*."""
    return "".join([random.choice(choices) for dummy in range(length)])
...
class TestImageColorize:
    ...
    def test_colorize_la_rgb(self):
        ...
        from .test_xrimage import TestXRImageColorize
