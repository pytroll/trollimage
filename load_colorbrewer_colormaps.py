...
def main():
    ...
    _print_colormap_group(cmap_groups["qual"], "Qualitative", "qualitative_colormaps", normalize_values=False)
...
def _load_colormap_info_from_colorbrewer() -> dict[str, dict[str, list]]:
    with urllib.request.urlopen(JSON_URL) as json_file:  # nosec: B310
        ...
        for cmap_name, cmap_info in colorbrewer_dict.items():
            max_colors = max(
                (num_colors_str for num_colors_str in cmap_info if num_colors_str != "type"),
                key=lambda num_color_str: int(num_color_str),
            )
            cmap_colors = cmap_info[max_colors]
            color_tuples = [
                rgb_color_str.replace("rgb(", "").replace(")", "").split(",") for rgb_color_str in cmap_colors
            ]
            cmap_groups[cmap_info["type"]][cmap_name.lower()] = color_tuples
    return cmap_groups


def _print_colormap_group(
    cmap_group: dict[str, list], human_group_name: str, group_var_name: str, normalize_values: bool = True
) -> None:
...
def _color_info_as_human_friendly_strings(
    ...
    color_human_strings = [
        (f"{rgb_color[0]} / 255", f"{rgb_color[1]} / 255", f"{rgb_color[2]} / 255") for rgb_color in cmap_colors
    ]
...
def _print_single_colormap(cmap_name: str, cmap_values: list[str], cmap_colors: list[tuple[str, str, str]]) -> None:
    ...
    for cmap_value_str, cmap_color_tuple in cmap_pairs:
        print(f"    ({cmap_value_str}, ({cmap_color_tuple[0]}, {cmap_color_tuple[1]}, {cmap_color_tuple[2]})),")
    print(")\n")
